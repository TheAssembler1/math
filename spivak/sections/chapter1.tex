...
\begin{proof}
    \begin{align*}
        x^2 + bx + c 
            &= \left(\frac{-b + \sqrt{b^2 - 4c}}{2}\right)^2 + b\left(\frac{-b + \sqrt{b^2 - 4c}}{2}\right) + c \\
            &= \frac{b^2 - 2b\sqrt{b^2 - 4c} + b^2 - 4c}{4} + \frac{-2b^2 + 2b\sqrt{b^2 - 4c}}{4} + \frac{4c}{4} \\
            &= \frac{2b^2 - 2b^2 + 2b\sqrt{b^2 - 4c} - 2b \sqrt{b^2 - 4c} + 4c - 4c}{4} \\
            &= \frac{0}{4} = 0
    \end{align*}
\end{proof}
...
\begin{proof}
    Suppose $b^2 - 4 c < 0$.
    For contradiction, suppose $x^2 + bx + c = 0$ for some $x$.
    Then $x^2 + bx + c = 0 
        \iff x^2 + bx = -c 
        \iff \left(x + \frac{b}{2}\right)^2 = \left(\frac{b}{2}\right)^2 - c 
        \iff \left(x + \frac{b}{2}\right)^2 = \frac{b^2 - 4c}{4}$.
    Now, $b^2 - 4c < 0$ thus $\frac{b^2 - 4c}{4} < 0$
        contradicting the square of a non-zero number being $>0$.
\end{proof}

\begin{proof}
    Suppose wlog that $x \ne 0$.
    Let $b = y$ and $c = y^2$.
    Notice $b^2 - 4c = y^2 - 4y^2$ which is certainly $<0$.
    It directly follows from part (b)
        that $x^2 + xy + y^2 > 0$.
\end{proof}

\begin{proof}
    From part (b), for $x^2 + \alpha xy + y^2$ to be $>0$ for all $x,y$ not both $0$,
        we require that $(\alpha y)^2 - 4y^2 < 0$.
    Then
        $(\alpha y)^2 - 4y^2 < 0
            \iff (\alpha y)^2 < 4y^2
            \iff \alpha^2 y^2 < 4y^2
            \iff \alpha^2 < 4$.
    From which it follows that $|\alpha| < 2$.
\end{proof}

\begin{proof}
    We find the smallest value of $ax^2 + bx + c$ to derive the formula for the general solution 
        then show $x^2 + bx + c$ as a specific case where $a = 1$.
    Notice 
    \begin{align*}
        ax^2 + bx + c 
    ...
    \end{align*}
    It is clear that $x = -\frac{b}{2a}$ gives the minimum value.
    Plugging in shows $\min(ax^2 + bx + c) = c - \frac{b^2}{4a}$.
    Let $a = 1$, plugging in shows $\min(x^2 + bx + c) = c - \frac{b^2}{4}$.
\end{proof}
...
\begin{proof}
    ...
    Then 
    \[0 = 0 + 0
        = x_1 \cdot 0 + x_2 \cdot 0 
        = x_1 y_1 + x_2 y_2 = \sqrt{x_1^2 + x_2^2} \cdot 0 
        = \sqrt{x_1^2 + x_2^2} \sqrt{0}         
        = \sqrt{x_1^2 + x_2^2} \sqrt{0^2 + 0^2}
        = \sqrt{x_1^2 + x_2^2}\sqrt{y_1^2 + y_2^2}\]
    Thus $x_1 x_2 = \sqrt{x_1^2 + x_2^2}\sqrt{y_1^2 + y_2^2}$ as required.
\end{proof}
...
\begin{proof}
    Suppose $|x - x_0| < \frac{\epsilon}{2} \text{ and } |y - y_0| < \frac{\epsilon}{2}$.
    Then 
    \[|(x + y) - (x_0 + y_0)| = |(x - x_0) + (y - y_0)| 
        \le |x - x_0| + |y - y_0| < \frac{\epsilon}{2} + \frac{\epsilon}{2} = \epsilon\]
    Thus $|(x + y) - (x_0 + y_0)| < \epsilon$.
    Similarly 
    \[|(x - y) - (x_0 - y_0)| = |(x - x_0) + (y_0 - y)| 
        \le |x - x_0| + |y_0 - y| = |x - x_0| + |y - y_0| < \frac{\epsilon}{2} + \frac{\epsilon}{2} = \epsilon\]
    Thus $|(x - y) - (x_0 - y_0)| < \epsilon$.
\end{proof}
...
\begin{tcolorbox}[title=Problem 21, breakable]
    ...
    about $x - x_0$ and $y - y_0$, it is almost a forgone conclusion that the proof will depend
...
\begin{proof}
    Suppose $y_0 \not = 0$.
    Then 
    \[|y| = |y - y_0 + y_0| = |y_0 + (y - y_0)| \ge ||y_0| - |y - y_0|| \ge |y_0| - |y - y_0| > |y_0| - \frac{|y_0|}{2} = \frac{|y_0|}{2}\]
    Thus $y \ne 0$.
    Also 
    \[\left|\frac{1}{y} - \frac{1}{y_0}\right|
    =\left|\frac{y - y_0}{y y_0}\right|
    = \frac{|y - y_0|}{|y||y_0|}
    < \frac{\frac{\epsilon|y_0|^2}{2}}{\frac{|y_0|}{2}|y_0|}
    = \epsilon\]
\end{proof}
