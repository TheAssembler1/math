...
\begin{proof}
    ...
    For all $n$
    \[|x_n - y_n| \le |x_n| + |y_n| \le X + Y,\]
    and 
    \[x_n y_n \le XY.\]
    To see $B$ is not an ideal, take $(1, 1, 1, \ldots) \in B$
        and $(1, 2, 3, \dots) \in S$.
    Then $b \cdot s = (1, 2, 3, \dots) \notin B$.
    Thus $B$ is not an ideal of $S$.
\end{proof}
...
\begin{tcolorbox}[title=Problem 12, breakable]
    ...
    a different symbol $y$ for the new indeterminate, gives us
...
\textbf{Solution (a): }
\[\langle x, y \rangle = \{f(x, y)x + g(x, y)y \mid f(x, y), g(x, y) \in \mathbb{Q}[x, y]\}.\] 
Notice $\langle x,y \rangle$ consists of the zero-polynomial along with polynomials with a zero constant term in $\mathbb{Q}[x, y]$.

\begin{proof}
    Suppose that $\langle x, y \rangle$ is principal ideal generated from $f \in \mathbb{Q}[x, y]$.
    Thus 
    \[\langle f \rangle = \langle x, y \rangle = \{f \cdot g \mid g \in \mathbb{Q}[x, y]\}.\]
    Since $x \in \langle f \rangle$ and $y \in \langle f \rangle$
        we must have $x = f g$ and $y = f g'$ for some $g, g' \in \mathbb{Q}[x, y]$.
    Now $deg(x) = deg(f) + deg(g)$ and since $deg(x) = 1$ we have $deg(f) \le 1$.
    Now, if $deg(f) = 0$ then $\langle f \rangle = \mathbb{Q}[x,  y]$ thus $deg(f) = 1$.
    Therefore let $f = \alpha x + \beta y + \rho$ for some $\alpha, \beta, \rho \in \mathbb{Q}$.
    We know that $\langle x, y \rangle$ consists of polynomials with a zero constant thus $\rho = 0$.
    So we have $f = \alpha x + \beta y$.
    Now $x = fg = (\alpha x + \beta y)g$. The lhs has no $y$ term thus $\beta = 0$.
    So $f = \alpha x$. But $y = f g' = (\alpha x) g'$.
    But $deg_x(y) \ne deg_x((\alpha x) g') = 1$ which is a contradiction.
    Thus $\langle x, y \rangle$ is not a principal ideal.
\end{proof}