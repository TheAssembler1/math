...
\begin{proof}
    ...
    Also 
    \[cx = c(a_1 x_1 + a_2 x_2 + \cdots + a_n x_n) = a_1 (c x_2) + a_2 (c x_2) + \cdots + a_n (c x_n) \in I.\]
    $I$ is closed under scalar multiplication thus $c x_i \in R$.
    Thus $I$ is an ideal of $R$.
    Suppose $I'$ is an ideal containing all $a_i$'s.
    Then $I'$ is closed under scalar multiplication by elements of $R$
...
We see 
\[\langle x^n \rangle \subset \langle x^{n - 1} \rangle \subset \cdots \subset \langle x \rangle \subset \langle 1 \rangle = \mathbb{Q}[x].\]
Now, $\mathbb{Q}[x]$ is a PID and is thus Noetherian.
By the well ording principle the chain of subsets is finite and thus doesn't contradict $\mathbb{Q}[x]$ being Neotherian.
...
\begin{proof}
    ...
    Let $T$ be an arbitrary element in $I$.
    There exists $A \in \mathcal{P}(X)$ such that $C \cap A = T$.
    Thus $C \subseteq T$.
    But then every $x \in C$ is in $T$ and is thus in $I$ and $C \ne \emptyset$.
    Thus $C = \mathcal{P}(X)$ contradicting that $I$ has only finite sets.
\end{proof}
...
\begin{proof}
    ...
    Similarly
    \[cx = c(x_1, x_2, \cdots, x_n, 0_m, 0_{m + 1}, \cdots) = (c x_1, c x_2, \cdots, c x_n, c \cdot 0_m, c \cdot 0_{m + 1}, \cdots) = (c x_1, c x_2, \cdots, c x_n, 0_m, 0_{m + 1}, \cdots) \in I_n.\]
...
\begin{proof}
    The proof is identical to part $1$.
    We have 
    \[\sum \subset I_n.\]
\end{proof}
...
\begin{proof}
    Let $x, y$ be arbitrary elements in $B$.
    Let $c$ be an arbitrary element in $B$.
\end{proof}
