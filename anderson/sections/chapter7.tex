...
\begin{proof}
    ...
    [(a, b)(c, d)](e, f)
    &= (ac + da + bc, bd)(e, f) \\
    &= \big((ac + da + bc)e + f(ac + da + bc) + (bd)e,\; (bd)f\big) \\
    &= \big(a(ce) + a(de) + b(ce) + a(fc) + a(fd) + b(fc),\; b(df)\big) \\
    &= (a, b)(ce + fc + de,\; df) \\
    &= (a, b)[(c, d)(e, f)].
    \end{aligned}
    \]
    ...
    (a, b)\big[(c, d) + (e, f)\big]
    &= (a, b)(c + e, d + f) \\
    &= \big(a(c + e) + (d + f)a + b(c + e),\; b(d + f)\big) \\
    ...
    \big[(a, b) + (c, d)\big](e, f)
    &= (a + c, b + d)(e, f) \\
    &= \big((a + c)e + f(a + c) + (b + d)e,\; (b + d)f\big) \\
...
\begin{proof}
    Let $(r,n) \in R \times \mathbb{Z}$. Then
    \[
...
\begin{proof}
    Let $R$ be a ring. Consider the subset $R \times \{0\} = \{(r,0) \mid r \in R\} \subseteq R \times \mathbb{Z}$.  
    Let $(r,0), (s,0)$ be arbitrary elements in $R \times \{0\}$.  
    Since $r, s \in R$, it follows that $(r,0) - (s,0) = (r - s, 0) \in R \times \{0\}$.  
    Also, since $r, s \in R$, it follows that $(r,0)(s,0) = (rs + 0\cdot r + 0\cdot s, 0\cdot 0) = (rs, 0) \in R \times \{0\}$.  
    Finally, note that $(0_R, 0) \in R \times \{0\}$, so the subset is nonempty.  
    By Theorem 7.1, $R \times \{0\}$ is a subring of $R \times \mathbb{Z}$.  
    The map $r \longrightarrow (r,0)$ preserves addition and multiplication, so $R \times \{0\}$ behaves exactly like $R$.  
\end{proof}