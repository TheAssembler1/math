...
\begin{tcolorbox}[title=Problem 8, breakable]
    ...
    (c) Suppose that $n_0 \ge 2$ and the arithmetic mean-geometric mean 
        ...
        there are positive real number $a_1, a_2, \ldots, a_n$ such that 
        \[\frac{a_1 + a_2 + \cdots + a_n}{2^n} < \sqrt[2^n]{a_1 a_2 \ldots a_n}\]
...
\newpage
\begin{proof}
    (\textbf{Base Case}) Let $a_1, a_2$ be positive real numbers.
    By part(a) it follows that
    \[(a + b)/2 \ge \sqrt{ab}\]

    (\textbf{Induction Step}) Suppose for some $n \ge 1$ that
    \[
    \frac{a_1 + a_2 + \cdots + a_{2^n}}{2^n} \ge \sqrt[2^n]{a_1 a_2 \cdots a_{2^n}}.
    \]
    Now consider $2^{n+1}$ positive real numbers $a_1,\dots,a_{2^{n+1}}$ and group them into
    $2^n$ pairs:
    \[
    (a_1,a_2),\,(a_3,a_4),\dots,(a_{2^{n+1}-1},a_{2^{n+1}}).
    \]
    For each pair define
    \[
    m_k = \frac{a_{2k-1} + a_{2k}}{2} \qquad (k=1,\dots,2^n).
    \]
    By the $n=1$ case we have $m_k \ge \sqrt{a_{2k-1} a_{2k}}$ for each $k$. Therefore
    \[
    m_1 m_2 \cdots m_{2^n} \ge \sqrt{a_1 a_2 \cdots a_{2^{n+1}}}.
    \]
    Applying the induction hypothesis to the numbers $m_1,\dots,m_{2^n}$ we get
    \[
    \frac{m_1 + \cdots + m_{2^n}}{2^n} \ge \sqrt[2^n]{m_1 m_2 \cdots m_{2^n}}
    \ge \sqrt[2^{n+1}]{a_1 a_2 \cdots a_{2^{n+1}}}.
    \]
    But
    \[
    \frac{m_1 + \cdots + m_{2^n}}{2^n}
    = \frac{a_1 + a_2 + \cdots + a_{2^{n+1}}}{2^{n+1}}.
    \]
...
\begin{proof}
    (\textbf{Base Case}) Supose $A$ is a set with a single element.
    ...
    (\textbf{Induction Step}) Suppose the theorem holds for sets with $n + 1$ elements.
    Suppose $A$ is a set with $n + 1$ elements and $f : A \rightarrow A$.
    For contradiction, suppose $f$ is one-to-one but not onto. 
    Then there is some $a \in A$ such that $a \notin Ran(f)$.
    Let $A' = A \setminus \{a\}$ and $f'= f \cap (A' \times A')$.
\end{proof}
