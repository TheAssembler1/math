...
\begin{proof}
    ...
    \begin{align*}
        y^2 &= ca^2 - a^3 \\
        \iff y^2 &= a^2(c - a) \\
        \iff y &= \pm \sqrt{a^2(c - a)} \\
        \iff y &= \pm a \sqrt{c - a}
    \end{align*}
    If $c < a$ then there is no solution.
    If $c = a$ then $y = 0$ thus there is a single solution $(0, 0)$.
    If $c > a$ then there are two solutions given by $\pm a \sqrt{c - a}$.
    ...
    \begin{align*}
        y^2 &= cx^2 - x^3 \\
        \iff (mx + b)^2 &= cx^2 - x^3 \\
        \iff m^2 x^2 + 2 mxb + b^2 &= cx^2 - x^3 \\
        \iff x^3 + m^2 x^2 - c x^2 + 2 mxb + b^2 &= 0 \\
        \iff x^3 + (m^2 - c)x^2 + (2mb) x + b^2 &= 0
    \end{align*}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        \iff x^3 + (m^2 - c)x^2 &= 0
    \end{align*}
    Now $x = 0$ is a solution so we need $x + (m^2 - c) = 0$.
    Thus our single solutions is $x = -(m^2 - c)$.

    Looking at the figure the slope of the going up to the right when $x > 0$
        is smaller than that of the slop of the line going to the bottom left when $x < 0$.
    Thus if a line were to intersect within the circle its slope would be 
        incorrect to increcept the lines when $x < 0$.
\end{proof}
...
\begin{proof}
    When viewing the figure we can clearly see any point in the range is 
        obtainable by taking $t$ to be sufficiently large or small.
    Thus all points on the curve can be reached.
